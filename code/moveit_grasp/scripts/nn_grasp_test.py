...
box_size_dict =  { 4:[ 0.215, 0.05,0.095],3:[0.13,0.07,0.075],2:[ 0.155,0.05, 0.055],1:[ 0.1,0.03, 0.13],0:[ 0.16,0.045, 0.17]}
...
for objects in all_object_data:
    ...
    for object in objects:
        ...
        output_classes = network.feed_batch([image_color])
        ...
        print box_size_dict[class_index]
        moveit.grasp(x,y,z,degrees(np.mean(predictions)), box_size_dict[class_index])
    #moveit.move_to(0.07762650150394189,0.19728139122386762,0.18509897914268225,92.5137252764)
    #if not flag:
     #   moveit.move_to(0,0,0,degrees(np.mean(predictions)))